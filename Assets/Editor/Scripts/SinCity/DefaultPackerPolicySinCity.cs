...
public class DefaultPackerPolicySinCity : UnityEditor.Sprites.IPackerPolicy
{
    ...
    public bool AllowSequentialPacking { get { return true; } }
    ...
    public void OnGroupAtlases(BuildTarget target, UnityEditor.Sprites.PackerJob job, int[] textureImporterInstanceIDs)
    {
        ...
        foreach (int instanceID in textureImporterInstanceIDs)
        {
            ...
            foreach (Sprite sprite in sprites)
            {
                ...
                if (desiredFormat == TextureFormat.PVRTC_RGBA2 || desiredFormat == TextureFormat.PVRTC_RGBA4)
                {
                    if (sprite.textureRect.width > 1024 || sprite.textureRect.height > 1024)
                    {
                        Debug.LogError("Sprite size to big for PVRTC_RGBA ATLASS: sprite:"+sprite.name + "; w:"+sprite.textureRect.width.ToString() + "; h:"+sprite.textureRect.height.ToString());
        ...
        foreach (var atlasGroup in atlasGroups)
        {
            ...
            foreach (var settingsGroup in settingsGroups)
            {
                ...
                //SINCITY IOS PVRTC RGBA ATLAS SETTINGS!!!==>
                bool isPVRTC_RGBA = (settings.format == TextureFormat.PVRTC_RGBA2 || settings.format == TextureFormat.PVRTC_RGBA4);
                if (isPVRTC_RGBA)
                {
